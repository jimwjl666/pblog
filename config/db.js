...
var tableConf=require('../config/table');
var settings = require('../settings')
var options = {
 ...
 * @param callback 回调方法
 */
DB.prototype.findAndPage = function (table_name, conditions, pageParams,orderby) {
    var node_model = this.getConnection(table_name);
    if(pageParams){
        return (node_model.find(conditions).skip((pageParams.pageIndex-1)*pageParams.pageSize).limit(pageParams.pageSize).sort(orderby||{}).exec());
    }
    return (node_model.find(conditions).exec());
};
...
DB.prototype.count = function (table_name, conditions, callback) {
    var node_model = this.getConnection(table_name);
    return node_model.count(conditions).exec();
